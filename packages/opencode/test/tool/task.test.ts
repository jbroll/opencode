...
describe("tool.task", () => {
  ...
  it.instance("execute without model override uses agent configured model", () =>
    Effect.gen(function* () {
      const { chat, assistant } = yield* seed()
      const tool = yield* TaskTool
      const def = yield* tool.init()
      ...
      expect(seen?.model?.providerID).toBe(ProviderV2.ID.make("agent-provider"))
  ...
  it.instance("execute without model override falls back to parent model for agent with no configured model", () =>
    Effect.gen(function* () {
      const { chat, assistant } = yield* seed()
      const tool = yield* TaskTool
      const def = yield* tool.init()
